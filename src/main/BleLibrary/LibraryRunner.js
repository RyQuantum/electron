...
} = require('blelocklibrary/NotificationConstants');
...
let globalAccessToken =
  '"eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJwYXJ0bmVySWQiOjEsImRldmljZU1hYyI6IkY1OkM0OkVGOkRFOkQ4Ojk0Iiwicm9sZSI6IkFETUlOIiwiaWF0IjoxNTk3NDk2NDM2LCJleHAiOjE1OTc1ODI4MzZ9.QSiggszbIdVkHMo49PjVeMcM01aHOTttFt6igPEKWR0';
const validity = 1;
...
const blueLogin = async (username, password) => {
  try {
    ...
    if (response.data) {
      const { data: { token: { accessToken, expiresAt } = {} } = {} } =
        response;
...
class LibraryRunner extends EventEmitter {
  constructor() {
    ...
    const commandList = [this.packetLossCheck];
    ...
    this.foundDevice = async (device) => {
      ...
      setTimeout(async () => {
        for (const keyValue of tests) {
          console.log(`*** Running test: ${keyValue[0]}`);

          try {
            for (const command of keyValue[1]) {
  ...
  makeTests() {
    ...
    this._tests.set('lockUnlock', [
      this.lock,
      this.unlock,
      this.lock,
      this.unlock,
    ]);
    this._tests.set('lockUnlockPause', [
      this.lock,
      this.pause,
      this.unlock,
      this.pause,
  ...
  async notification() {
    ...
    if (this.createdDevice) {
      try {
        const response = await this.createdDevice.notification(
          NotificationTypeEnum.EKEY
        );
  ...
  initLibrary(userId = 1) {
    ...
    this.library = new OaksBleLockLibrary(
      this.getToken,
      this._blePlugin,
      this._persistencePlugin
    );
  ...
  selectDevice(lockMac) {
    this.createdDevice = this.devices.find(
      (device) => device.lockMac === lockMac
    );
  }
  ...
  unlock = async () => {
    ...
    if (this.createdDevice) {
      const response = await this.createdDevice.unlock();
      console.log(response);
      this.emit('unlockRes', response);
    } else {
      console.log('No device created');
    }
  };
  ...
  lock = async () => {
    ...
    if (this.createdDevice) {
      const response = await this.createdDevice.lock();
      console.log(response);
      this.emit('lockRes', response);
    } else {
      console.log('No device created');
    }
  };
  ...
  async forceUnlock() {
    ...
    if (this.createdDevice) {
      const response = await this.createdDevice.forceUnlock();
      console.log(response);
    } else {
      console.log('No device created');
    }
  ...
  async forceLock() {
    ...
    if (this.createdDevice) {
      const response = await this.createdDevice.forceLock();
      console.log(response);
    } else {
      console.log('No device created');
    }
  ...
  async setLockTime() {
    ...
    if (this.createdDevice) {
      const response = await this.createdDevice.setLockTime();
      console.log(response);
    } else {
      console.log('No device created');
    }
  ...
  readLockTime = async () => {
    ...
    if (this.createdDevice) {
      const response = await this.createdDevice.getLockTime();
      console.log(response);
      ...
      if (this.createdDevice) {
        for (i = 0; i < 1; i++) {
          ...
          if (response.success) {
            ...
            response[Object.keys(response)[0]][1] ===
            'Error Occurred: Unknown error occurred'
          )
            pktSentCount++;
          console.log('pktSentCount=====', pktSentCount);
        }
        console.log(
          'Packet loss percentage is = ',
          ((20 - pktSentCount) / 20) * 100
        );
      } else {
        console.log('No device created');
      }
  ...
  async resetLock() {
    ...
    if (this.createdDevice) {
      const response = await this.createdDevice.resetLock();
      console.log(response);
    } else {
      console.log('No device created');
    }
  ...
  async initLock() {
    ...
    if (this.createdDevice) {
      const response = await this.createdDevice.addAdministrator();
      console.log(response);
    } else {
      console.log('No device created');
    }
  ...
  async addFob() {
    ...
    if (this.createdDevice) {
      ...
      const startTime = '00:00:08';
      const endTime = '23:59:00';
      const response = await this.createdDevice.addICCard(
        undefined,
        undefined,
        startTime,
        endTime,
  ...
  async addFobWithNumber() {
    ...
    if (this.createdDevice) {
      ...
      const startTime = '00:00:08';
      const endTime = '23:59:00';
      const response = await this.createdDevice.addICCardWithNumber(
        undefined,
        undefined,
        startTime,
        endTime,
  ...
  async deleteFobWithNumber() {
    ...
    if (this.createdDevice) {
      const response = await this.createdDevice.deleteICCard(57, 'C33902B5');
  ...
  async addPeriodPasscode() {
    ...
    if (this.createdDevice) {
      ...
      const response = await this.createdDevice.addPeriodPasscode(
        startDate.getTime(),
        endDate.getTime(),
        '1234',
        'test code'
  ...
  async addCyclicPasscode({
    ...
    if (this.createdDevice) {
      ...
      const response = await this.createdDevice.addCyclicPasscode(
        startDate,
        endDate,
        startTime,
        endTime,
      ...
      console.log(response);
      this.emit('addCyclicPasscodeRes', response);
    } else {
      console.log('No device created');
    }
  }
  ...
  async deletePasscode() {
    ...
    if (this.createdDevice) {
      const response = await this.createdDevice.deletePasscode(775, '12345678');
  ...
  async getLog() {
    console.log('\n--- trying to get Logs ----\n');
    if (this.createdDevice) {
      const response = await this.createdDevice.getLog();
  ...
  async setAutoLockTime() {
    ...
    if (this.createdDevice) {
      ...
      const response = await this.createdDevice.setAutoLockTime(0);
  ...
  async setDoorSensorLocking() {
    ...
    if (this.createdDevice) {
      const response = await this.createdDevice.setDoorSensorLocking(true);
  ...
  async isDoorSensorEnabled() {
    ...
    if (this.createdDevice) {
      const response = await this.createdDevice.isDoorSensorEnabled();
  ...
  async getAutoLockTime() {
    ...
    if (this.createdDevice) {
      const response = await this.createdDevice.getAutoLockTime();
  ...
  async getBattery() {
    ...
    if (this.createdDevice) {
      const response = await this.createdDevice.getBattery();
...
const run = async () => {
  const response = await blueLogin('rently', 'rentlySecret');
